...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512):
        ...
        cached_features_file = os.path.join(
            directory, args.data_version + '_cached_lm_' + str(block_size) + '_' + filename)
    ...
    def read_nraws(self):
        self.num_nraws += 1
        logger.info("Reading the [%d]th data block from dataset file at %s" % (
            self.num_nraws, self.file_path))
        ...
        for idx, token in enumerate(doc_tokens):
            ...
            if self.args.model_type == 'roberta':
                sub_tokens = self.tokenizer.tokenize(
            ...
            for sub_w in sub_tokens:
                ...
                if ori_token != token:
                    ...
                    if ori_token.lower() in self.term2ids_dict:
                        adv_labels.append(
                            self.term2ids_dict[ori_token.lower()])
                    else:
        ...
        for i in range(0, len(tokenized_text)-seq_maxlen+1, seq_maxlen):
            input_ids = self.tokenizer.build_inputs_with_special_tokens(
                tokenized_text[i:i+seq_maxlen])
            labels = [-1] + mask_labels[i:i+seq_maxlen] + \
                [-1]  # For CLS and SEP
            adv_input_labels = [-1] + adv_labels[i:i+seq_maxlen] + [-1]
            char_input_ids, start_ids, end_ids = self.build_char_inputs(
                input_ids, sub_index_to_orig_token, i, self.rng, labels)
    ...
    def create_masked_lm_predictions(self, tokens, masked_lm_prob, tokenizer, rng, sub_index_to_change):
        ...
        for p in masked_lms:
            ...
            masked_lm_labels[p.index] = tokenizer.convert_tokens_to_ids(
                p.label)
    ...
    def create_adv_word(self, orig_token, rng):
        ...
        if len(orig_token) < 4:
            ...
            if self.args.output_debug and False:
                print(
                    f"Insert the char:{rand_char} orig_token: {orig_token} new_token: {token}")
        else:
            if rng.random() < 0.5:
                ...
                if self.args.output_debug and False:
                    print(
                        f"Delete the char:{orig_token[rand_idx]} orig_token: {orig_token} new_token: {token}")
            else:
                idx = random.randint(1, len(orig_token)-2)
                token[idx], token[idx+1] = token[idx+1], token[idx]
                if self.args.output_debug and False:
                    print(
                        f"Swap the char:{token[idx:idx+2]} orig_token: {orig_token} new_token: {token}")
        token = ''.join(token)
        return token
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            outputs = model(char_input_ids, start_ids, end_ids,
                            inputs, masked_lm_labels=labels, adv_labels=adv_labels)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_dataloader = DataLoader(
        eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=1)
