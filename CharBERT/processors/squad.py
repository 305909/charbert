...
def squad_convert_examples_to_features(
    ...
    for (example_index, example) in enumerate(tqdm(examples, desc="Converting examples to features")):
        ...
        if model_type == 'roberta':
            truncated_query = tokenizer.encode(
                example.question_text, add_special_tokens=False, max_length=max_query_length, add_prefix_space=True, truncation=True
            )
        sequence_added_tokens = tokenizer.model_max_length - \
    ...
    if return_dataset == "pt":
        ...
        all_start_ids = torch.tensor(
            [f.start_ids for f in features], dtype=torch.long)
        all_end_ids = torch.tensor(
            [f.end_ids for f in features], dtype=torch.long)
