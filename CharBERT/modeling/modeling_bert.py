...
class CharBertEncoder(nn.Module):
    def __init__(self, config, is_roberta=False):
        ...
        if not is_roberta:
            fusion_layer = torch.nn.Conv1d(
                in_channels=config.hidden_size * 2, out_channels=config.hidden_size, kernel_size=3, padding=3//2)
            self.fusion_layer_list = nn.ModuleList(
                [fusion_layer for _ in range(config.num_hidden_layers)])
        else:
            self.fusion_layer = torch.nn.Conv1d(
                in_channels=config.hidden_size * 2, out_channels=config.hidden_size, kernel_size=3, padding=3//2)

        self.act_layer = ACT2FN[config.hidden_act]
        self.word_norm = BertLayerNorm(
            config.hidden_size, eps=config.layer_norm_eps)
        self.char_norm = BertLayerNorm(
            config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, char_hidden_states, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask)
            char_layer_outputs = layer_module(
                char_hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask)
...
class BertOnlyMLMHead(nn.Module):
    ...
    def forward(self, sequence_output, char_sequence_output):
        mlm_scores, term_scores = self.predictions(
            sequence_output, char_sequence_output)
        return mlm_scores, term_scores
